...
# 原理
从编译原理来看，正则表达式匹配不是简单地写一个函数，而是需要把正则表达式编译成 NFA，再编译成 DFA，最后再进行匹配

所以 V8 这里的代码看着也会很怪，因为这里写的不是正则表达式匹配的代码，而是如何操作编译器，把正则表达式编译成机器码的代码

# 实现细节
## 执行流程概览
这边理应是一个 mermaid 图，但是因为图太大了，直接在这里预览会看不清，所以去[这里]看吧
